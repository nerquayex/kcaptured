    ...
    try {
      const result = await pool.query(
        ...
        [
          ...
          email,
          phone,
          body.packageName ? String(body.packageName).trim() : "",
          body.preferredDate ? new Date(body.preferredDate) : null,
          status,
      ...
      if (duplicate.rows[0])
        return json({
          booking: mapRow(duplicate.rows[0]),
          alreadyCreated: true,
        });
      throw error;
    }
...
export async function PATCH(request: Request) {
  ...
  try {
    const body = await request.json();
    if (!body.id || !validStatus(body.status))
