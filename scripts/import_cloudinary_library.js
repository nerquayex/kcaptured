...
const FOLDER_DEFAULT = process.env.IMPORT_FOLDER_DEFAULT || 'client-uploads'
...
async function fetchResources(prefix, next_cursor) {
  const opts = { resource_type: 'image', type: 'upload', max_results: pageSize, context: true, tags: true }
  ...
  try {
    ...
    do {
      ...
      for (const r of resources) {
        ...
        try {
          if (r.context && r.context.custom && r.context.custom.category) category = r.context.custom.category
        } catch (e) {}
        if (!category) {
        ...
        toInsert.push({ public_id, secure_url, width, height, category, title })
      }
    ...
    for (const item of toInsert) {
      // normalize public_id
      if (typeof item.public_id === 'string') item.public_id = item.public_id.replace(/\.[a-zA-Z0-9]+$/,'')
      // skip if no url
      if (!item.secure_url) { skipped++; continue }
      if (!doInsert) { console.log('[DRY] Would insert', item.public_id); continue }

      // check exists
      const existing = await pool.query('SELECT id FROM portfolio_items WHERE public_id = $1 LIMIT 1', [item.public_id])
      ...
      const r = await pool.query('SELECT COALESCE(MAX(sort_order), 0) as max FROM portfolio_items')
    ...
    console.log(`Done. scanned=${scanned} inserted=${inserted} skipped=${skipped}`)
    await pool.end()
